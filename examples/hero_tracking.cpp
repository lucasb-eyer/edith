#ifndef _HERO_TRACKING_VISITOR_CSVS_H
#define _HERO_TRACKING_VISITOR_CSVS_H
...
class HeroTrackingVisitorCSVs : public Visitor {
public:
    HeroTrackingVisitorCSVs(std::string basepath = "./")
        : _basepath(basepath)
        , _all((basepath + "all.csv").c_str())
    { }

    virtual ~HeroTrackingVisitorCSVs()
    {
...
protected:
    bool handle_entity(const Entity &hero) {
        ...
        try {
            ...
            if(iFile == _files.end()) {
                _files[heroname] = new std::ofstream((_basepath + heroname + ".csv").c_str());
